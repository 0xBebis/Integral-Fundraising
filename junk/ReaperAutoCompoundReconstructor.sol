 ...
 */
contract ReaperAutoCompound is Ownable, Pausable {
    ...
    mapping (address => bool) public approvedRewards;
    ...
    function approveReward(address _reward) external onlyOwner returns (bool) {
      approvedRewards[_reward] = true;
      return true;
    }

    function approveRouter(address _router) external onlyOwner returns (bool) {
      approvedRouters[_router] = true;
      return true;
    }

    function updateReward(address _reward) internal returns (bool) {
      rewardToken = _reward;
      return true;
    }

    function updateRouter(address _router) internal returns (bool) {
      uniRouter = _router;
      return true;
    }

    function reInitialize(address lpPair, address reward, address router) public onlyStrategist returns (bool) {
      removeAllowances();
      updateReward(reward);
      updateRouter(router);
